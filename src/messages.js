module.exports = {
  not_authorized: 'You are not authenticated!',

  account_problem: 'Error occured with your account, retry or report to support service',
  already_exists: 'User with this email already exists',
  no_user_email: 'No user with that email',
  incorrect_password: 'Incorrect password',

  no_simulation_id: 'No simulation with this id found',
  simulation_already_ran: 'Simulation has been ran, you can relaunched it or create a new one to have new parameters',
}
